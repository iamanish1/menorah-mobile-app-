...
export const QUERY_KEYS = {
  ...
  bookings: (userId: string, params?: object) =>
    ["bookings", userId, params ?? {}] as const,
...
export function useSpecializations() {
  return useQuery({
    ...
    select: (res) =>
      res.success ? ((res.data?.specializations ?? []) as string[]) : [],
  });
}
...
export function useLanguages() {
  return useQuery({
    ...
    staleTime: 30 * 60 * 1000,
    select: (res) =>
      res.success ? ((res.data?.languages ?? []) as string[]) : [],
  });
}
...
export function useChatRooms() {
  ...
  return useQuery({
    ...
    select: (res) =>
      res.success ? ((res.data?.chatRooms ?? []) as ChatRoom[]) : [],
  });
}
