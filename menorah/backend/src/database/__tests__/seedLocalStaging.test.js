...
const {
  ADMIN_ROLE_GRANTS,
  COUNSELLOR_IDS,
  LOCAL_STAGING_EMAIL_DOMAIN,
  LocalStagingSeedError,
...
const makeEnv = () => {
  const env = {
    ...
    MENORAH_LOCAL_STAGING_SEED_CONFIRM: SEED_CONFIRMATION,
    ...
    MENORAH_STAGING_EMAIL_DOMAIN: LOCAL_STAGING_EMAIL_DOMAIN,
    MONGO_INITDB_DATABASE: 'menorah',
    MONGODB_REPLICA_SET_NAME: 'menorah-rs',
    MONGODB_URI:
      'mongodb://local-seed:unit-only-credential@mongo-primary:27017/menorah?authSource=admin&replicaSet=menorah-rs&retryWrites=true',
...
describe('local staging synthetic seed target guards', () => {
  test('accepts only the complete expected local staging identity', () => {
  ...
  test.each([
    ...
    ['MENORAH_LOCAL_STAGING_SEED_CONFIRM', 'true'],
    ['MENORAH_SYNTHETIC_DATA_ONLY', 'TRUE'],
    ['COMPOSE_PROJECT_NAME', 'menorah'],
    ['MENORAH_LOCAL_STAGING_ENVIRONMENT_ID', 'menorah-local-staging-v2'],
    ['MENORAH_STAGING_EMAIL_DOMAIN', 'staging.localhost'],
    ['MONGO_INITDB_DATABASE', 'menorah_local_staging'],
    ['MONGODB_REPLICA_SET_NAME', 'menorah-local-staging-rs'],
  ])('rejects a non-exact %s target value', (key, value) => {
    const env = makeEnv();
    env[key] = value;
    expect(() => assertSeedAllowed(env)).toThrow(
      expect.objectContaining({ code: 'LOCAL_STAGING_TARGET_GUARD_FAILED' })
    );
  });
  ...
  test.each([
    ...
    expect(() => assertSeedAllowed(env)).toThrow(
      expect.objectContaining({
        code: 'LOCAL_STAGING_CANDIDATE_IDENTITY_INVALID',
      })
    );
  });
  ...
  test.each([
    [
      'wrong host',
      'mongodb://local-seed:unit-only-credential@mongo:27017/menorah?authSource=admin&replicaSet=menorah-rs',
    ],
    [
      'non-exact host casing',
      'mongodb://local-seed:unit-only-credential@MONGO-PRIMARY:27017/menorah?authSource=admin&replicaSet=menorah-rs',
    ],
    [
      'wrong port',
      'mongodb://local-seed:unit-only-credential@mongo-primary:27018/menorah?authSource=admin&replicaSet=menorah-rs',
    ],
    [
      'wrong database',
      'mongodb://local-seed:unit-only-credential@mongo-primary:27017/production?authSource=admin&replicaSet=menorah-rs',
    ],
    [
      'wrong auth source',
      'mongodb://local-seed:unit-only-credential@mongo-primary:27017/menorah?authSource=menorah&replicaSet=menorah-rs',
    ],
    [
      'wrong replica set',
      'mongodb://local-seed:unit-only-credential@mongo-primary:27017/menorah?authSource=admin&replicaSet=other-rs',
    ],
    [
      'missing credentials',
      'mongodb://mongo-primary:27017/menorah?authSource=admin&replicaSet=menorah-rs',
    ],
    [
      'SRV target',
      'mongodb+srv://local-seed:unit-only-credential@mongo-primary/menorah?authSource=admin&replicaSet=menorah-rs',
    ],
    [
      'extra target option',
      'mongodb://local-seed:unit-only-credential@mongo-primary:27017/menorah?authSource=admin&replicaSet=menorah-rs&directConnection=true',
    ],
    [
      'disabled retryable writes',
      'mongodb://local-seed:unit-only-credential@mongo-primary:27017/menorah?authSource=admin&replicaSet=menorah-rs&retryWrites=false',
    ],
    [
      'duplicate target option',
      'mongodb://local-seed:unit-only-credential@mongo-primary:27017/menorah?authSource=admin&authSource=admin&replicaSet=menorah-rs',
    ],
  ])('rejects a Mongo URI with %s', (_label, uri) => {
    expect(() => assertMongoTarget(uri)).toThrow(
      expect.objectContaining({
        code: 'LOCAL_STAGING_MONGODB_TARGET_INVALID',
      })
    );
  });
  ...
  test.each([
    ...
    expect(() => assertSeedAllowed(env)).toThrow(
      expect.objectContaining({
        code: 'LOCAL_STAGING_COUNSELLOR_POLICY_INVALID',
      })
    );
  });
  ...
  test.each([
    ...
    expect(() => assertSeedAllowed(env)).toThrow(
      expect.objectContaining({
        code: 'LOCAL_STAGING_ADMIN_GRANTS_INVALID',
      })
    );
  });
  ...
  test.each([
    ...
    expect(() => assertSeedAllowed(env)).toThrow(
      expect.objectContaining({ code: 'LOCAL_STAGING_PASSWORD_INVALID' })
    );
  });
  ...
  test('rejects a shared password before roster construction', () => {
    ...
    expect(() => assertSeedAllowed(env)).toThrow(
      expect.objectContaining({ code: 'LOCAL_STAGING_PASSWORD_REUSED' })
    );
  });
  ...
  test('refuses before connecting when any guard fails', async () => {
    ...
    await expect(run({ env, mongooseInstance })).rejects.toMatchObject({
      code: 'LOCAL_STAGING_TARGET_GUARD_FAILED',
    });
...
describe('local staging synthetic roster', () => {
  test('has the exact bounded shape and only clearly synthetic profile data', () => {
    ...
    userDocuments.forEach((document) => {
      expect(document.email.endsWith(`@${LOCAL_STAGING_EMAIL_DOMAIN}`)).toBe(
        true
      );
    ...
    roster.applications.forEach(({ document }) => {
      ...
      expect(document.credentialEvidence[0].reference).toMatch(
        /^urn:menorah:synthetic-local-staging:/
      );
...
describe('local staging synthetic roster writes', () => {
  test.each([
    ['user', 'UserModel'],
    ...
    async (_label, collisionModel) => {
      ...
      await expect(seedSyntheticRoster({
        ...
        code: 'LOCAL_STAGING_ROSTER_ALREADY_PRESENT',
      });
  ...
  test('checks every roster ID, email, phone, and counsellor license before inserting', async () => {
      ...
      { _id: { $in: Object.values(USER_IDS) } },
      {
        email: {
          $in: expect.arrayContaining([
            `user-a@${LOCAL_STAGING_EMAIL_DOMAIN}`,
            `admin-full-2@${LOCAL_STAGING_EMAIL_DOMAIN}`,
          ]),
        },
      },
      {
    ...
    expect(counsellorFilter.$or[2].licenseNumber.$in).toEqual([
      'SYNTHETIC-LOCAL-COUNSELLOR-A',
      'SYNTHETIC-LOCAL-COUNSELLOR-DRAFT',
      'SYNTHETIC-LOCAL-COUNSELLOR-SUSPENDED',
    ]);
  ...
  test('reduces unexpected CLI failures to a value-free error code', () => {
    expect(safeErrorCode({
      message:
        'duplicate key for user-a@mail.staging.localhost and +12025550100',
    })).toBe('UNEXPECTED_ERROR');
    expect(safeErrorCode(new LocalStagingSeedError(
      'LOCAL_STAGING_ROSTER_ALREADY_PRESENT',
      'value-free controlled failure'
    ))).toBe('LOCAL_STAGING_ROSTER_ALREADY_PRESENT');
    expect(safeErrorCode({
