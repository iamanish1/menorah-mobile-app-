...
const LOCAL_STAGING_EMAIL_DOMAIN = 'mail.staging.localhost';
const LOCAL_STAGING_PROJECT = 'menorah-local-staging';
const LOCAL_STAGING_ENVIRONMENT_ID = 'menorah-local-staging-v1';
const LOCAL_STAGING_MONGO_HOST = 'mongo-primary:27017';
const LOCAL_STAGING_DATABASE = 'menorah';
const LOCAL_STAGING_REPLICA_SET = 'menorah-rs';
...
const PASSWORD_ENV_BY_ALIAS = Object.freeze({
  'USER-A': 'MENORAH_LOCAL_STAGING_USER_A_PASSWORD',
  'USER-B': 'MENORAH_LOCAL_STAGING_USER_B_PASSWORD',
  'COUNSELLOR-A': 'MENORAH_LOCAL_STAGING_COUNSELLOR_A_PASSWORD',
  'COUNSELLOR-DRAFT':
    'MENORAH_LOCAL_STAGING_COUNSELLOR_DRAFT_PASSWORD',
  'COUNSELLOR-SUSPENDED':
    'MENORAH_LOCAL_STAGING_COUNSELLOR_SUSPENDED_PASSWORD',
  'ADMIN-SUPPORT': 'MENORAH_LOCAL_STAGING_ADMIN_SUPPORT_PASSWORD',
  'ADMIN-FINANCE': 'MENORAH_LOCAL_STAGING_ADMIN_FINANCE_PASSWORD',
  'ADMIN-CONTENT': 'MENORAH_LOCAL_STAGING_ADMIN_CONTENT_PASSWORD',
  'ADMIN-FULL-1': 'MENORAH_LOCAL_STAGING_ADMIN_FULL_1_PASSWORD',
  'ADMIN-FULL-2': 'MENORAH_LOCAL_STAGING_ADMIN_FULL_2_PASSWORD',
});
...
class LocalStagingSeedError extends Error {
  constructor(code, message) {
    super(message);
    this.name = 'LocalStagingSeedError';
    this.code = code;
  }
}

const fail = (code, message) => {
  throw new LocalStagingSeedError(code, message);
};

const requireExactEnvironmentValue = (env, key, expected) => {
  if (env?.[key] !== expected) {
    fail(
      'LOCAL_STAGING_TARGET_GUARD_FAILED',
      `${key} does not identify the approved local staging target.`
    );
  }
};
...
const assertMongoTarget = (rawUri) => {
  if (
    ...
    fail(
      'LOCAL_STAGING_MONGODB_TARGET_INVALID',
      'MONGODB_URI is not an exact local staging MongoDB target.'
    );
  }
  ...
  const exactAuthorityAndDatabase = new RegExp(
    `^mongodb://[^/?#]+@${LOCAL_STAGING_MONGO_HOST}`
      + `/${LOCAL_STAGING_DATABASE}\\?`
  ).test(rawUri);
  if (
    parsed.protocol !== 'mongodb:'
    || !exactAuthorityAndDatabase
    || parsed.host !== LOCAL_STAGING_MONGO_HOST
    || parsed.pathname !== `/${LOCAL_STAGING_DATABASE}`
    || !parsed.username
    || !parsed.password
    || parsed.hash
    || !boundedParameterSet
    ...
    || parsed.searchParams.get('replicaSet') !== LOCAL_STAGING_REPLICA_SET
...
const assertAdminRoleGrants = (rawGrants) => {
  ...
  if (
    ...
    fail(
      'LOCAL_STAGING_ADMIN_GRANTS_INVALID',
...
const assertSeedAllowed = (env = process.env) => {
  ...
  requireExactEnvironmentValue(
    ...
    LOCAL_STAGING_REPLICA_SET
  );

  if (!/^[a-f0-9]{40}$/.test(env?.MENORAH_RUNTIME_CANDIDATE_SHA || '')) {
    fail(
      'LOCAL_STAGING_CANDIDATE_IDENTITY_INVALID',
  ...
  if (
    ...
    fail(
      'LOCAL_STAGING_COUNSELLOR_POLICY_INVALID',
...
const counsellorProfile = ({ alias }) => ({
  licenseNumber: `SYNTHETIC-LOCAL-${alias}`,
...
const buildApplication = ({
  ...
  return {
    ...
    credentialEvidence: [{
      _id: evidenceId,
      reference: `urn:menorah:synthetic-local-staging:${alias.toLowerCase()}`,
      category: 'synthetic-fixture',
      submittedAt: reviewedAt,
      source: 'synthetic_local_staging',
      review: {
        decision: 'approved',
        policyVersion: env.COUNSELLOR_CREDENTIAL_POLICY_VERSION,
        reviewedBy: adminId,
...
const buildRoster = ({
  ...
  if (!(now instanceof Date) || !Number.isFinite(now.getTime())) {
    fail(
      'LOCAL_STAGING_SEED_TIME_INVALID',
  ...
  const users = ACCOUNT_SPECS.map((spec) => {
    const document = {
      ...
      email: `${spec.emailLocalPart}@${LOCAL_STAGING_EMAIL_DOMAIN}`,
      phone: spec.phone,
...
const safeErrorCode = (error) => (
  error instanceof LocalStagingSeedError
    ? error.code
    : 'UNEXPECTED_ERROR'
);
...
if (require.main === module) {
  run().catch((error) => {
    console.error(
      `Local staging synthetic roster seed failed (${safeErrorCode(error)}).`
    );
    process.exitCode = 1;
  });
}
